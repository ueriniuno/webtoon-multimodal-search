...
RESULT_PATH = os.path.join(BASE_DIR, "webtoon-multimodal-search", "eval_results_scored104_RR_difficulty.json")
...
def load_results(path: str) -> pd.DataFrame:
    ...
    # difficulty가 scored 결과에 없으면 raw(RR)에서 id로 보강
    if "difficulty" not in df.columns and "id" in df.columns and os.path.exists(RAW_PATH):
        try:
            with open(RAW_PATH, "r", encoding="utf-8") as f:
                raw = json.load(f)
            raw_df = pd.DataFrame(raw)
            if "id" in raw_df.columns and "difficulty" in raw_df.columns:
                df = df.merge(
                    raw_df[["id", "difficulty"]],
                    on="id",
                    how="left",
                )
        except Exception as e:
            print(f"[WARN] difficulty 머지 실패: {e}")
...
def plot_score_by_chapter(df: pd.DataFrame):
    if "gt_chapter" not in df.columns or "eval_score" not in df.columns:
        return
...
def plot_score_vs_chapter_ok(df: pd.DataFrame):
    if "chapter_ok" not in df.columns or "eval_score" not in df.columns:
        return
...
def plot_accuracy_by_difficulty(df: pd.DataFrame):
    """
    difficulty(질문 난이도)별 정답률(= chapter_ok 평균)을 % 비율로 시각화
    """
    if "chapter_ok" not in df.columns or "difficulty" not in df.columns:
        return

    # 난이도를 숫자로 정렬 가능한 형태로 변환 (예: "1","2","3"...)
    diff_series = pd.to_numeric(df["difficulty"], errors="coerce")
    df = df.copy()
    df["difficulty_num"] = diff_series

    # 난이도별 정답률 (chapter_ok 평균)
    acc_df = (
        df.dropna(subset=["difficulty_num"])
        .groupby("difficulty_num")["chapter_ok"]
        .mean()
        .reset_index()
        .sort_values("difficulty_num")
    )
    # 비율(0~1)을 %로 변환
    ...
    acc_df["difficulty_label"] = acc_df["difficulty_num"].apply(get_difficulty_label)
    ...
    plt.ylim(0, 100)
    plt.title("difficulty by accuracy (%)")
...
def plot_difficulty_ratio(df: pd.DataFrame):
    """
    difficulty 분포 자체(난이도별 샘플 비율)를 시각화
    """
    ...
    ratio_df.columns = ["difficulty_num", "ratio"]
    ...
    ratio_df["difficulty_label"] = ratio_df["difficulty_num"].apply(get_difficulty_label)
    ...
    plt.ylim(0, 1)
    plt.title("난이도(difficulty)별 샘플 비율")
    plt.xlabel("difficulty")
    plt.ylabel("비율")
    plt.tight_layout()
